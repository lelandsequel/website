...
// FactoryTour — a self-contained, dependency-free PRESENTATION walkthrough of the
// /factory page (Stage 3 — the OMNIS build leg). A title slide → spotlight stops →
// a closing slide, with optional AUTO-PLAY (timed advance + progress bar) so it can
// run itself like a deck for a live demo, or be driven by hand. Spotlights anchor
// to real DOM via [data-tour="…"]; everything is inline-styled against /factory's
// own DARK tokens (light ink on near-black, periwinkle #9fb4ff primary, green
// #6fe0a8 for "shipped/closed", Geist Mono labels) so it reads native to the page.
//
// Drive it: ▶/❚❚ play-pause · ← → / Enter step · Space play-pause · R replay · Esc close.
//
// Honesty discipline (load-bearing): the validator catches a REAL miss and the
// agent self-corrects — no claim is dressed up beyond what the page proves. The
// one stated edge (per-story deliverability signal) is not hidden by the tour.

import { useCallback, useEffect, useLayoutEffect, useState } from "react";

const LS_KEY = "factory-tour-v1-seen";
const MONO = "var(--font-mono), ui-monospace, monospace";
const SANS = "var(--font-sans), Inter, system-ui, sans-serif";
const ACCENT = "#9fb4ff"; // periwinkle — page primary
const ACCENT_HI = "#c7d0ff"; // brighter on the dark slide
const SHIP = "#6fe0a8"; // green — shipped / closed
const INK = "#e7e7ea"; // light text
const MUTED = "#9a9aa4"; // secondary text
...
const STEPS = [
  {
    kind: "cover",
    kicker: "JourdanLabs · OMNIS — the build leg",
    title: "Three stages. One governed loop. Closed.",
    sub: "Watch software get decided, specified, built, and gated — then watch the loop re-decide on what the build actually proved. The one thing nobody else ships: a validator that can't lie about whether the work is right.",
  },

  { kind: "step", n: 1, act: "I · The factory", sel: '[data-tour="stages"]',
    title: "Not two products side by side — three stages of one loop.",
    body: "Agility decides WHAT to build. 6D specs HOW. The build leg — this page — BUILDS it and GATES it. The third stage is the one everyone skips, because it's the hard one: deterministic, refusal-gated code, not a diagram." },

  { kind: "step", n: 2, act: "II · The work order", sel: '[data-tour="workorder"]',
    title: "A 6D story is already a build work-order.",
    body: "Its acceptance criteria aren't prose to admire — they're compiled into executable probes. The validator runs each one against whatever the builder delivers. The builder isn't graded on its own say-so; it's graded on the criteria, by something it can't talk out of a 'no.'" },

  { kind: "step", n: 3, act: "III · Build + gate", sel: '[data-tour="loop"]',
    title: "The agent built it. The validator refused it. Then it fixed itself.",
    body: "Round 1: the build priced on stale data — a real, dangerous miss. The gate caught it, named the failing criterion, and handed it back. Round 2: the agent resolved it and re-gated to green. REFUSE → RESOLVE → RECOMPUTE, every round sealed in a tamper-evident chain. The builder is a commodity; THIS gate is the product." },

  { kind: "step", n: 4, act: "IV · The circle closes", sel: '[data-tour="backtoagility"]',
    title: "What the build proved goes back to Agility.",
    body: "6D fed back the real effort. The build leg feeds back proven deliverability: a shipped build CONFIRMS the estimate; one the gate can't pass DE-RISKS the initiative and the portfolio re-decides. Decide → specify → build → re-decide on what was measured, not guessed — a receipt riding the whole circle." },

  {
    kind: "outro",
    kicker: "The loop that proves the loop",
    title: "AI that can't lie about whether the work is right.",
    sub: "All three stages — governed, deterministic, a receipt at every step — and a validator no one else has. This is the OMNIS software factory, closed.",
  },
];
